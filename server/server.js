...
async function checkVersion() {
  return new Promise((resolve) => {
    // 桌面端/受限网络下 npm registry 可能长时间无响应; 必须设超时, 否则
    // serveNcmApi 的 Promise.all 会一直卡住, HTTP 端口永远不会 listen。
    const child = exec(
      'npm info NeteaseCloudMusicApiEnhanced version',
      { timeout: 5000 },
      (err, stdout) => {
        if (!err) {
...
async function serveNcmApi(options) {
  ...
  ╩ ╩╩  ╩    ╚═╝╝╚╝╩ ╩╩ ╩╝╚╝╚═╝╚═╝═╩╝
    ...
    server.on('error', (error) => {
      logger.error(
        `Server failed to listen on ${host || '0.0.0.0'}:${port}`,
        error,
      )
      reject(error)
    })
    appExt.server = server
  })
